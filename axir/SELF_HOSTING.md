...
The bootstrap has a verified direct-ELF proof: it parses modules, links them, validates the merged program, removes no-op moves, expands configured bytes, relocates static data and `rel32` branches, writes a standalone ELF64 image itself, and executes AXIR `write` plus `exit` sequences. It does not invoke an assembler or linker. This is deliberately only a backend subset (`mov` immediate, immediate `add`/`sub`, `xor`, `addr`, `jmp`, `cjump`, primitive syscall, and `hlt` over `I[0..2]`); it is not a complete v1.0 backend or a self-hosted compiler. Complete multi-unit symbol semantics, register allocation/spills, all remaining operations, calls, floating point, writable data/BSS, and runtime execution support remain unimplemented. Stages 2–5 are not implemented.
